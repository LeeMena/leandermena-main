        ...
        <div className="container">
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-6)' }}>
            {trustBadges.map((badge) => (
              <span key={badge} style={{ fontSize: '0.72rem', letterSpacing: '0.13em', textTransform: 'uppercase', color: 'var(--color-text-muted)', opacity: 0.6 }}>
                {badge}
              </span>
            ))}
          </div>
      ...
      <section className="section" style={{ background: 'var(--color-surface)' }}>
        <div className="container">
          ...
          >
            ...
            <p className="section-intro" style={{ marginBottom: 0 }}>
              SOPs, playbooks, and courses distilled from 18+ years operating Miami&rsquo;s top hospitality venues.
            </p>
          </motion.div>
      ...
      <section className="section">
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-6)', marginBottom: 'var(--space-6)' }}>
            {approvedTestimonials.slice(0, 2).map((t, i) => (
              <TestimonialCard key={t.name + i} testimonial={t} index={i} featured />
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 'var(--space-6)' }}>
            {approvedTestimonials.slice(2, 5).map((t, i) => (
          ...
          <div style={{ textAlign: 'center', marginTop: 'var(--space-8)' }}>
            <Link to="/case-studies" className="btn btn-secondary">
              Read Full Case Studies <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-16)', alignItems: 'center' }}>
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              ...
              <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.7, marginBottom: 'var(--space-8)', fontSize: '0.95rem' }}>
                Most hospitality groups don&rsquo;t need another full-time executive. They need a seasoned operator
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', marginBottom: 'var(--space-8)' }}>
                {[
                  ...
                  <li key={item.title} style={{ display: 'flex', gap: 'var(--space-4)', alignItems: 'flex-start' }}>
                    <div style={{ width: '38px', height: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(184,160,128,0.10)', color: 'var(--color-primary)', borderRadius: 'var(--radius-sm)', flexShrink: 0 }}>
                      {item.icon}
                    </div>
                    <div>
              ...
              <Link to="/about" className="btn btn-primary">
                Learn More About Leander <ArrowRight size={16} />
              </Link>
            </motion.div>
        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-8)' }}>
            {[
              ...
              >
                <span style={{ fontFamily: 'var(--font-serif)', fontSize: '3.5rem', color: 'rgba(184,160,128,0.18)', lineHeight: 1, display: 'block', marginBottom: 'var(--space-3)' }}>
                  {item.step}
                </span>

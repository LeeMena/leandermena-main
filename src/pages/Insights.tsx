  ...
  return (
    <Layout seo={{
      ...
      <section className="relative min-h-[50vh] flex items-end pb-20 overflow-hidden bg-luxury-black">
        ...
        <div className="container-luxury relative z-10 pt-32">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="font-display text-5xl md:text-6xl lg:text-7xl text-luxury-text mb-6">Insights</h1>
            <div className="divider-gold mb-6 ml-0" />
            <p className="font-serif text-lg text-luxury-muted/80 italic max-w-2xl leading-relaxed">
              Weekly operations intelligence for hospitality leaders.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-luxury py-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gold/10 flex items-center justify-center text-gold"><BookOpen className="w-5 h-5" /></div>
              <div>
            ...
            <Link to="/book" className="btn-primary text-[10px] py-3 px-6">Subscribe to Newsletter</Link>
          </div>
        </div>
      </section>
              ...
              <motion.article key={post.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-[10px] text-luxury-muted flex items-center gap-1"><Clock className="w-3 h-3" />{post.readTime}</span>
                  </div>
                  <h3 className="font-serif text-xl text-luxury-text mb-3 group-hover:text-gold transition-colors">{post.title}</h3>
                  <p className="text-sm text-luxury-muted leading-relaxed mb-4 line-clamp-3">{post.excerpt}</p>
                  <div className="flex flex-wrap gap-2">
                    {post.tags.slice(0, 3).map((tag, j) => (
                      <span key={j} className="px-2 py-1 bg-luxury-dark text-[10px] text-luxury-muted uppercase tracking-wider">{tag}</span>
                    ))}
                  </div>
                </div>
              </motion.article>
            ...
            {categories.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`px-4 py-2 text-[10px] tracking-[0.15em] uppercase transition-all ${
                  activeCategory === cat ? 'bg-gold text-luxury-black' : 'border border-luxury-border text-luxury-muted hover:border-gold hover:text-gold'
                }`}>{cat}</button>
            ))}
          </div>
          <div className="space-y-6">
            {filteredPosts.map((post, i) => (
              <motion.article key={post.id} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex-1">
                  ...
                  <h3 className="font-serif text-xl text-luxury-text mb-2 group-hover:text-gold transition-colors">{post.title}</h3>
                  <p className="text-sm text-luxury-muted leading-relaxed mb-3 line-clamp-2">{post.excerpt}</p>
                  <div className="flex flex-wrap gap-2">
                    {post.tags.map((tag, j) => (<span key={j} className="px-2 py-1 bg-luxury-dark text-[10px] text-luxury-muted uppercase tracking-wider">{tag}</span>))}
                  </div>
                </div>
              </motion.article>
            ))}
      ...
      <CTABanner
        ...
        subtitle="Join 2,400+ hospitality leaders receiving actionable strategies every Tuesday."
